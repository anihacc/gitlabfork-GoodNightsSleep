...
public class GNSTagProv
{
	public static class BlockTagProv extends BlockTagsProvider
	{
		...
		void vanilla()
		{
			...
			ForgeRegistries.BLOCKS.getValues().stream().filter(block -> block.getRegistryName().getNamespace().equals(GoodNightSleep.MODID) && block instanceof StairBlock && !block.defaultBlockState().getMaterial().equals(Material.WOOD)).forEach(this.tag(BlockTags.STAIRS)::add);
			...
			ForgeRegistries.BLOCKS.getValues().stream().filter(block -> block.getRegistryName().getNamespace().equals(GoodNightSleep.MODID) && block instanceof StairBlock && block.defaultBlockState().getMaterial().equals(Material.WOOD)).forEach(this.tag(BlockTags.WOODEN_STAIRS)::add);
