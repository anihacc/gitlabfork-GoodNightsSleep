...
public class BlockGNSBed extends BlockDirectional
{
    ...
    public void teleportPlayerDream(boolean shouldSpawnPortal, EntityPlayer player) 
	{
		if (player instanceof EntityPlayerMP)
		{
			...
			//if (ForgeHooks.onTravelToDimension(this.player, transferDimension))
			{
				...
				{
					transferEntity(shouldSpawnPortal, player.riddenByEntity, server.worldServerForDimension(previousDimension), server.worldServerForDimension(transferDimension));
					player.riddenByEntity.mountEntity(null);
				}

				server.getConfigurationManager().transferPlayerToDimension((EntityPlayerMP) player, transferDimension, teleporter);

				if (player.ridingEntity != null)
				{
					transferEntity(shouldSpawnPortal, player.ridingEntity, server.worldServerForDimension(previousDimension), server.worldServerForDimension(transferDimension));
				}
			}
		}
	}

	public void teleportPlayerNightmare(boolean shouldSpawnPortal, EntityPlayer player) 
	{
		if (player instanceof EntityPlayerMP)
		{
			int previousDimension = player.dimension;
