...
public class GNSBigTreeFeature extends AbstractTreeFeature<TreeFeatureConfig>
{
	...
	private void foliageCluster(IWorldGenerationReader worldIn, BlockPos pos, MutableBoundingBox p_202393_3_, Set<BlockPos> set1)
	{
		for (int i = 0; i < 5; ++i)
		{
			this.crossSection(worldIn, pos.up(i), this.foliageShape(i), p_202393_3_, set1);
		}
	}
	...
	private void makeFoliage(IWorldGenerationReader worldIn, int p_208525_2_, BlockPos pos, List<GNSBigTreeFeature.FoliageCoordinates> p_208525_4_, MutableBoundingBox p_208525_5_, Set<BlockPos> set1)
	{
			...
			for (; i1 >= 0; --i1)
			{
				...
				if (!(f < 0.0F))
				{
					for (int j1 = 0; j1 < k; ++j1)
					{
						...
						if (this.makeLimb(set1, worldIn, blockpos, blockpos1, false, box) == -1)
						{
							...
							if (this.makeLimb(set1, worldIn, blockpos2, blockpos, false, box) == -1)
							{
	...
	{
		if (!isSoil(p_208528_2_, p_208528_3_.down(), (IPlantable) GNSBlocks.dream_sapling))
		{
			return -1;
		}
		else
	...
	protected static boolean func_214587_a(IWorldGenerationBaseReader p_214587_0_, BlockPos p_214587_1_)
	{
		if (!(p_214587_0_ instanceof net.minecraft.world.IWorldReader))
			return p_214587_0_.hasBlockState(p_214587_1_, (p_214573_0_) ->
			{
				...
				return p_214573_0_.isAir() || p_214573_0_.isIn(BlockTags.LEAVES) || block == GNSBlocks.dream_grass_block || block == GNSBlocks.nightmare_grass_block || block == GNSBlocks.nightmare_grass_block || block == Blocks.DIRT || block == Blocks.GRASS_BLOCK || block.isIn(BlockTags.LOGS) || block.isIn(BlockTags.SAPLINGS) || block == Blocks.VINE;
